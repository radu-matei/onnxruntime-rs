...
use ndarray::Array;
use sys::{OrtAllocator, OrtSession};
...
#[derive(Debug)]
pub struct OwnedSessionBuilder {
    env: Environment,
    session_options_ptr: *mut sys::OrtSessionOptions,
...
impl Drop for OwnedSessionBuilder {
    #[tracing::instrument]
    fn drop(&mut self) {
...
impl OwnedSessionBuilder {
    pub(crate) fn new(env: Environment) -> Result<OwnedSessionBuilder> {
    ...
    pub fn with_number_threads(self, num_threads: i16) -> Result<OwnedSessionBuilder> {
    ...
    pub fn use_cuda(self) -> Result<OwnedSessionBuilder> {
        unsafe {
    ...
    pub fn with_allocator(mut self, allocator: AllocatorType) -> Result<OwnedSessionBuilder> {
    ...
    pub fn with_memory_type(mut self, memory_type: MemType) -> Result<OwnedSessionBuilder> {
    ...
    #[cfg(feature = "model-fetching")]
    pub fn with_model_downloaded<M>(self, model: M) -> Result<OwnedSession>
    where
    ...
    #[cfg(feature = "model-fetching")]
    fn with_model_downloaded_monomorphized(
        self,
        model: AvailableOnnxModel,
    ) -> Result<OwnedSession> {
    ...
    pub fn with_model_from_file<P>(self, model_filepath_ref: P) -> Result<OwnedSession>
    where
        P: AsRef<Path>,
    {
    ...
    pub fn with_model_from_memory<B>(self, model_bytes: B) -> Result<OwnedSession>
    where
        B: AsRef<[u8]>,
    {
    ...
    fn with_model_from_memory_monomorphized(self, model_bytes: &[u8]) -> Result<OwnedSession> {
        let mut session_ptr: *mut sys::OrtSession = std::ptr::null_mut();
